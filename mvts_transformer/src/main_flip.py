...
from datasets.data_flip import data_factory, Normalizer
...
def main(config):
    ...
    df = pd.DataFrame({'pred_labels': predict_labels, 'target_labels': target_labels, 'score_A': predict_scores[:, 0], 'score_C': predict_scores[:, 1]})
    ...
    train_df = pd.DataFrame({'pred_labels': train_predict_labels, 'target_labels': train_target_labels, 'score_A': train_predict_scores[:, 0], 'score_C': train_predict_scores[:, 1]})
