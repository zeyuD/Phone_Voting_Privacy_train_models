...
df4.insert(len(df4.columns), 'Avg', '')
...
for user in user_list:
    ...
    for feature_name in feature_names:
        ...
        for t in range(num_try):
            # print("User: ", user)
            subprocess.run(["python", "src/main_perUser.py",
        ...
        df5.to_csv("acc_avg_"+feature_name+"_perUser.csv", index=False)
...
df_final.to_csv("acc_avg_"+feature_name+"_perUser.csv", index=False)