        ...
        for v in range(len(vote_list)):
            ...
            if self.use == 'train':
                for user in otherU_list:
                    ...
                    user_files = [file for file in os.listdir(train_path) if user in file and not file.startswith('.')]
                    files.extend(user_files)
            elif self.use == 'test':
                user_files = [file for file in os.listdir(train_path) if self.target_user in file and not file.startswith('.')]
                files.extend(user_files)
