...
def main(config):
    ...
    test_data = data_class('test', config['test_data_dir'], pattern=config['pattern'], n_proc=config['n_proc'], limit_size=config['limit_size'], config=config)
    ...
    # df1 = pd.read_csv('pred_labels.csv')

    # # print("Previous shape", df1.shape)
    
    # # Add a new row at the end to the empty dataframe
    # print("Length of predict labels", len(predict_labels))
    # print("Length of df columns", len(df1.columns))
    # df1.loc[df1.shape[0]] = predict_labels
    # df1.loc[df1.shape[0]] = target_labels
    # df1.loc[df1.shape[0]] = predict_scores[:, 0]
    # df1.loc[df1.shape[0]] = predict_scores[:, 1]
    # df1.loc[df1.shape[0]] = predict_scores[:, 2]
    # df1.loc[df1.shape[0]] = predict_scores[:, 3]
    # df1.loc[df1.shape[0]] = predict_scores[:, 4]
    # df1.to_csv('pred_labels.csv', index=False)
    ...
    # Save test results
    df2 = pd.read_csv('acc_avg.csv')
    # combine user name (string), accuracy (number), and average (number)
    new_row = pd.DataFrame([[config['target_user']] + acc.flatten().tolist() + [acc.mean()]], columns=df2.columns)
    print("New row added:", new_row)
    df2 = pd.concat([df2, new_row], ignore_index=True)
    df2.to_csv('acc_avg.csv', index=False)
