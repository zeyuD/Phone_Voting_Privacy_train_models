...
for feature_name in feature_names:
    ...
    df2 = pd.DataFrame(columns=['pred_labels', 'target_labels', 'score_A', 'score_C'])
    df2.to_csv("train_pred_labels.csv", index=False)
    df3 = pd.DataFrame(columns=['pred_labels', 'target_labels', 'score_A', 'score_C'])
    ...
    for t in range(num_try):
        # print("User: ", user)
        subprocess.run(["python", "src/main_2choice.py",
    ...
    df5.to_csv("acc_avg_"+feature_name+"_2choice.csv", index=False)
    ...
    df6.to_csv("pred_labels_"+feature_name+"_2choice.csv", index=False)

    df7 = pd.read_csv("train_pred_labels.csv")
    df7.to_csv("train_pred_labels_"+feature_name+"_2choice.csv", index=False)