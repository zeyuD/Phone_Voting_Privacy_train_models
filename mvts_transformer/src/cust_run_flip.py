...
for feature_name in feature_names:
    ...
    df2 = pd.DataFrame(columns=['pred_labels', 'target_labels', 'score_A', 'score_B', 'score_C'])
    df2.to_csv("train_pred_labels.csv", index=False)
    df3 = pd.DataFrame(columns=['pred_labels', 'target_labels', 'score_A', 'score_B', 'score_C'])
    ...
    for t in range(num_try):
        # print("User: ", user)
        subprocess.run(["python", "src/main_flip.py",
    ...
    df5.to_csv("acc_avg_"+feature_name+"_flip.csv", index=False)
    ...
    df6.to_csv("pred_labels_"+feature_name+"_flip.csv", index=False)

    df7 = pd.read_csv("train_pred_labels.csv")
    df7.to_csv("train_pred_labels_"+feature_name+"_flip.csv", index=False)