...
import pandas as pd
import os
from test_variables import feature_names, user_list, vote_list, num_try
import sys
...
for feature_name in feature_names:
    ...
    for t in range(num_try):
        # print("User: ", user)
        subprocess.run(["python", "run/test_main_5q.py",
    ...
    df5.to_csv("results/acc_avg_"+feature_name+"_5q.csv", index=False)
    ...
    df6.to_csv("results/pred_labels_"+feature_name+"_5q.csv", index=False)

    df7 = pd.read_csv("train_pred_labels.csv")
    df7.to_csv("results/train_pred_labels_"+feature_name+"_5q.csv", index=False)