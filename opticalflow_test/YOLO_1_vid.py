...
save_file = proj_path + "/data/" + setups["phone_s22"]["save_dir"] + votes[0] + "/downsample_480p/" + setups["phone_s22"]["users"][0] + "_" + votes[0] +"_1_optical_flow_480p.mp4"
# save_file = proj_path + "/data/" + setups["phone_s22"]["save_dir"] + votes[0] + "/downsample_480p/" + setups["phone_s22"]["users"][0] + "_" + votes[0] +"_1_optical_flow_gaussian_480p.mp4"
...
while True:
    ...
    if ret:
        ...
        # # Visualize edges and save to an image
        # edges_img = cv2.cvtColor(edges, cv2.COLOR_GRAY2RGB)
        # cv2.imwrite("results/edges.jpg", edges_img)
        results = YOLO_model(frame, verbose=False)
        # Visualize YOLO results and save to an image
        yolo_img = results[0].plot()
        cv2.imwrite("results/yolo.jpg", yolo_img)
